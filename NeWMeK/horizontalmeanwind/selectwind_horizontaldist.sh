...
psxy_shareopt="-JM14 -R${domain} -Ba1f1g1/a1f1g1 -SV0.05/0.15/0.15 -G0/0/255 -P -X0 -Y0"
#################################################################
########################## [ 設定事項 ] ##########################
#################################################################
...
if   [ "$(echo "${org_direction} >= 0" | bc -l)" -eq "1" -a "${station_num}" -ne "${e_station_num}" ]; then
  ...
  awk -F, '(NR=='${wind_read_counter}') {print '${awk_shareopt}'}' ${SELECT_WINDFILE} | psxy ${psxy_shareopt} -O >> ${IMAGE}
...
source ${GMT_TOOL_DIR}/horizontalmeanwind/selectwind_horizontaldist_function
#######################################################
